...
def create_graph_with_attributes(df, max_distance, coords, ener_label = 'ener'):
    '''
    ...
    for va, vb in itertools.combinations(graph.nodes(data=True), 2):
        ...
        if dis <= max_distance:
            graph.add_edge(va[0], vb[0], distance = dis)
    return graph

def create_graph_dict(voxels, max_distance, coords = ['x', 'y', 'z'], ener_label = 'ener'):
    '''
    ...
    for i, event in voxels.groupby('dataset_id'):
        g = create_graph_with_attributes(event, max_distance, coords, ener_label = ener_label)
        graphs[i] = g
    return graphs